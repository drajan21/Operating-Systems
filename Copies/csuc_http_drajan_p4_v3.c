...
int filecontent(char *path[], int newfp)
{

   // printf("in file content path:%s\n",*path);
    FILE *file; int fd;
    int f_size;
    ssize_t wbytes;
...
//            close(newfp);
          // wbytes=fwrite(buf,1,f_size,newfp);
  //         printf("path:%s wbytes:%d\n",*path,wbytes);
        }
        else
        {
            perror("Printing");
...
void sendresponse(int nsockfd,http_response_t http_response,char *fpath)
{
...
/*	if(http_response.status.code==HTTP_STATUS_LOOKUP[POSITION_NOTFOUND].code)
	...
	{
		file_fd=open(fpath,O_RDONLY,S_IRUSR);	
	}
   ...
   /* printf("Socket:%d\n",nsockfd);
	...
	if(numbytes==ERROR)
    {
     ...
     printf("%s\n%s %s\n",fpath,responsestatus,responseheader);
     printf("readfilebuf in send\n%s\n",readfilebuf);
    */

    filecontent(&fpath,nsockfd);
//	free(responseheader);
//	free(responsestatus);
//	close(file_fd);
//    close(nsockfd);
...
int main(int argc, char * argv[])
{
   ...
   // char *directory_name=malloc(sizeof(MEMORYSIZE));
    //directory_name="./";
    pthread_t tid;
    pthread_attr_t attr;
...
//            *ptr_sock=newsockfd; 
            int s = pthread_create(&tid,0,thread_exec,(void *)&newsockfd);
            if (s != 0) {
         ...
         //   free(read_request);
           /* void *res;
            int success=pthread_join(tid,&res);
            if(success!=0)
            {
                perror(NULL);
                exit(EXIT_FAILURE);
