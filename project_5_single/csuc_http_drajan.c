...
int send_stats()
{
    ...
    */
    ...
    logt(level,"\n\nStatistics-----------------------------------\n");
    logt(level,"Total Uptime                    : %f secs\n",time_diff);
    logt(level,"# of Requests handled           : %d\n",request_count);
...
char getinput(int argc, char *argv[],char * directory_name)
{
        ...
        case 'd': if(optarg!=NULL)
                  {
                      ...
                      if(resultdir==ERROR)
                      {
                          logt(level,"Directory Does Not Exist\n");
    ...
    {
        logt(level,"Inappropriate Options\n");
...
int dispatch_to_thread(int fd, char * name)
{
    ...
    *ptr_sock=fd;
    ...
    if (s != 0) 
    {
        logt(level,"Thread Creation Error\n");
